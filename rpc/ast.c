...
static int op_comparator(const void *a_, const void *b_)
{
	...
	return strncasecmp(a, b->key, strlen(b->key));
}
			 ...
			 char **token_val)
{
	...
	*token_val = token_str;
	...
	/* Look for summary operators, e.g.
	 *     min(MemAvailable)
	 */
	...
	/* Look for a keyword or attribute name, e.g.
	 *    from, select
	 */
	if (isalpha(*s)) {
		char keyword[255];
...
int ast_parse_group_by_clause(struct ast *ast, const char *expr, int *ppos)
{
	...
	/* Check that we don't have expressions */
	TAILQ_FOREACH(attr_e, &ast->select_list, link) {
		if (attr_e->expr) {
			ast->result = ASTP_SYNTAX;
			ast->pos = *ppos;
			snprintf(ast->error_msg, sizeof(ast->error_msg),
				 "The group clause is currently not supported "
				 "with expressions.\n");
			return ASTP_SYNTAX;
		}
	}
